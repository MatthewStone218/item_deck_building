...
function enemy_array_pick(){
	
	var count = array_length(_enemy_array);
	...
	for(var i = 0; i < count; i++)
	{
		ran_max += _enemy_array[i,1];
		_enemy_array[i,2] += ran_max;
	}
	//****************************************************************
	...
	for(var i = 0; i < count; i++)
	{
		if(_enemy_array[i,2] >= ran)
		{
			_return = _enemy_array[i,0];
			array_delete(_enemy_array,i,1);
			return _return;
		}
	}
