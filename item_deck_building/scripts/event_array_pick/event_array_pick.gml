...
function event_array_pick(){
	
	var count = array_length(_event_array);
	...
	for(var i = 0; i < count; i++)
	{
		ran_max += _event_array[i,1];
		_event_array[i,2] += ran_max;
	}
	//****************************************************************
	...
	for(var i = 0; i < count; i++)
	{
		if(_event_array[i,2] >= ran)
		{
			_return = _event_array[i,0];
			array_delete(_event_array,i,1);
			return _return;
		}
	}
